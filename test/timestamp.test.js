...
describe('timestamp api', function() {
  ...
  describe('valid date string param', function() {
  ...
  describe('invalid date string param', function() {

    const error = {
      'error': 'Invalid Date',
    };
    
    test('should return JSON with error key', done => {
      supertest(app)
        .get(apiURL + 'invalid_date_string')
        .expect(400, error)
        .end(done);
    });
